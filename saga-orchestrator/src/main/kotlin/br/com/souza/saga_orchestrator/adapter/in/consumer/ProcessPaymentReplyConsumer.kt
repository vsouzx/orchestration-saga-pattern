...
class ProcessPaymentReplyConsumer(
    ...
    fun consume(
        ...
        val span = tracer.spanBuilder("payments.replies.process-payment process")
        ...
        span.makeCurrent().use {
            try {
                logger.info("Received process-payment reply", kv("saga_id", reply.sagaId), kv("reply_status", reply.status))
                handleReply.execute(reply.sagaId, SagaStep.PROCESSING_PAYMENT_PENDING, ReplyStatus.valueOf(reply.status), reply.reason, traceParent)
            } finally {
                span.end()
            }
        }
