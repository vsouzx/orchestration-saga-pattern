...
class CancelOrderReplyConsumer(
    ...
    fun consume(
        ...
        val span = tracer.spanBuilder("orders.replies.cancel-order process")
        ...
        span.makeCurrent().use {
            try {
                logger.info("Received cancel-order reply", kv("saga_id", reply.sagaId), kv("reply_status", reply.status))
                handleReply.execute(reply.sagaId, SagaStep.CANCELING_ORDER_PENDING, ReplyStatus.valueOf(reply.status), reply.reason, traceParent)
            } finally {
                span.end()
            }
        }
