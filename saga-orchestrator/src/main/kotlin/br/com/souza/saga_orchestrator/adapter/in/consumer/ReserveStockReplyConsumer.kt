...
class ReserveStockReplyConsumer(
    ...
    fun consume(
        ...
        val span = tracer.spanBuilder("inventory.replies.reserve-stock process")
        ...
        span.makeCurrent().use {
            try {
                logger.info("Received reserve-stock reply", kv("saga_id", reply.sagaId), kv("reply_status", reply.status))
                handleReply.execute(reply.sagaId, SagaStep.RESERVING_STOCK_PENDING, ReplyStatus.valueOf(reply.status), reply.reason, traceParent)
            } finally {
                span.end()
            }
        }
